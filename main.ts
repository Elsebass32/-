...
namespace personajes {
    ...
    //% block="crear personaje con nombre %nombre salud %salud nivel %nivel imagen %imagen"
    //% inlineInputMode=inline
    export function crearPersonaje(nombre: string, salud: number, nivel: number, imagen: Image): Sprite {
        personajesMemoria[nombre] = { nombre: nombre, salud: salud, nivel: nivel }
        let personajeSprite = sprites.create(imagen, SpriteKind.Player)
